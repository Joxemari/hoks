...
function huecosPares(trazos, Wnom) {
  ...
  for (let k = 0; k < trazos.length; k++)
    for (let j = k + 1; j < trazos.length; j++) {
      ...
      // PROBADO AQUÍ Y REVERTIDO, que quede escrito. Al hacer que el apoyo naciera por el costado, su
      // cabo empezó a morir junto a una banda que sigue de largo, y ese encuentro —punta contra
      // costado— es invisible para este cálculo, que se salta el tramo si CUALQUIERA de los dos es un
      // cabo. El canal mínimo entre bandas se fue a 0,004 anchuras. Así que probé a saltar sólo punta
      // contra punta: el mínimo subió a 0,076, pero la banda de TODAS las obras adelgazó —mediana
      // 0,0635 → 0,0555— y el canal efectivo se fue a 0,376 cuando las cuatro van de 0,062 a 0,288.
      // O sea que arreglaba el punto más apretado echando blanco por toda la obra, que es justo lo
      // que él critica en las líneas gordas.
      //
      // El motivo es de escala y ya está escrito dos veces en este archivo: este suelo es GLOBAL y
      // recorta la banda de la obra entera, así que no puede gobernarlo un encuentro local. Un cabo
      // pegado a un costado es asunto de ESE punto, y de eso se ocupa el relleno, que mira punto a
      // punto. Ahí está el arreglo, no aquí.
      let m = Infinity;
...
const MANDOS = {
  ...
  dura:     3.0,         // cuánto tiene que DURAR una paralela para contar como acompañamiento, en
                         // anchuras de banda. El suelo estaba en unidades absolutas (1,4 pasos) y
                         // eso es la explicación de «con líneas finas algo de sentido tiene, tipo
                         // r1, r2. Pero en líneas gordas, tipo r5, aún estamos lejos»: 1,4 pasos son
                         // tres anchuras en una banda fina y UNA en una gorda, así que en las gordas
                         // se colaban trozos de un ancho de largo, que no se leen como que acompañan
                         // a nadie. La medida del ojo es la anchura, no el papel.
...
const ENV = { canal: 'HRRS_CANAL', obl: 'HRRS_OBL', tramo: 'HRRS_T', err: 'HRRS_ERR',
              largo: 'HRRS_L', vueltas: 'HRRS_V', dura: 'HRRS_DURA' };
function mandos(opt) {
...
function circuito(seed, opt) {
  ...
  const TIPOS = {
    ...
    // LA CUOTA DE PARALELAS BAJA DE 0,44 A 0,27 Y ESO CONSERVA SU VOTO, no lo deshace. La paralela
    // se construía por sorteo —treinta intentos a ciegas, y si un solo tramo estorbaba se tiraba la
    // paralela entera— así que de un 44 % pedido salía un 24 % real. Él votó mirando ESAS obras: lo
    // que eligió es la imagen del 24 %, no el número 44. Enumerando (ver la paralela, más abajo) el
    // mando por fin obedece y un 44 % pedido da un 38 % real: el acompañamiento se va a una mediana
    // del 68 % cuando en las seis es del 57 % y el máximo de las seis es 63. Pedir el 27 % devuelve
    // el 24 % real y la mediana al 57 %. Los dos argumentos —su voto y las referencias— caen en el
    // mismo sitio, que es la única razón para tocar una mezcla que él eligió.
    // Y EL APOYO CEDE 0,18 A LA SUELTA, porque el apoyo ya no es lo que era. Cuando llegaba de
    // frente contra el cuerpo no acompañaba a nadie, así que su cuota no contaba para la relación;
    // ahora nace por el costado y corre al lado del padre antes de morir, o sea que cuenta. Con la
    // cuota de antes la obra se pasaba: acompañamiento 67 % contra su 43-65, roturas por cambio de
    // socio 40 % contra su 21-36, y los cabos al aire a cero cuando en r1, r2 y r5 son el 17-19 %.
    // Traspasando 0,18 al que da aire, las cuatro medidas entran a la vez —63,6 / 37,5 / 5 % de
    // cabos al aire, y el 95 % de los cabos sigue muriendo contra algo— sin tocar el haz.
    denso:   { n: [8, 13], sep: [0.055, 0.080], cats: [0.27, 0.14, 0.23, 0.36] },
    abierto: { n: [5, 9],  sep: [0.085, 0.150], cats: [0.23, 0.14, 0.17, 0.46] },
  };
  ...
  const naceCat = (cat) => {
    ...
    if (cat === 'apoyo') {
      // EL APOYO LLEGA POR EL COSTADO, NO DE FRENTE. Nacía en el canal del padre y salía a
      // `q.dir + 90°`: perpendicular, o sea embistiéndolo. Muere contra el cuerpo —para eso está—
      // pero no lo acompaña ni una anchura antes de morir, así que toca sin juntarse.
      //
      // Y eso es lo que la auditoría acabó señalando. Midiendo los HACES —grupos de trazos que se
      // acompañan a distancia de canal durante dos anchuras seguidas— sus obras tienen UN haz que se
      // traga casi todo: 8 de 8 trazos en r1, 7 de 11 en r2, 9 de 14 en r6, más algún suelto de
      // verdad. Las nuestras se desmigaban en grupitos, con el haz mayor en 4. Y de los 78 trazos
      // que quedaban fuera de todo haz, 37 eran apoyos: ninguna paralela, porque la paralela se
      // construye pegada por definición.
      //
      // Así que el apoyo nace mirando A LO LARGO del padre y no contra él: arranca en el canal,
      // corre a su lado lo que su propio alfabeto le deje, y se va a morir donde le toque. Es la
      // diferencia entre tocar y encajarse — «las bandas de r5 se envuelven». No se convierte en una
      // paralela: la paralela es una copia rígida del padre y esto es un paseo, que empieza al lado
      // y se despega en cuanto el rumbo se le tuerce.
      const q = puntoDe(padre), lado = rng.bool(0.5) ? 1 : -1;
      return { p: [q.p[0] + Math.cos((q.dir + 90 * lado) * RAD) * sep * 1.10,
                   q.p[1] + Math.sin((q.dir + 90 * lado) * RAD) * sep * 1.10],
               dir: q.dir + (rng.bool(0.5) ? 0 : 180) + rng.range(-34, 34) };
    }
  ...
  for (let k = 1; k < n && !dado; k++) {
    ...
    // LA PARALELA SE CONSTRUYE, NO SE PASEA — Y SE ENUMERA, NO SE SORTEA.
    //
    // Aquí se sorteaba padre, lado y ventana, se comprobaba el offset entero y, si un solo tramo
    // estorbaba, se tiraba la paralela completa y se volvía a sortear. Treinta veces. El 80 % de
    // los intentos moría igual —el carril de un padre se ocupa en cuanto le sale la primera
    // paralela— así que sólo el 20 % de los trazos acababa siendo paralela pidiera la mezcla lo
    // que pidiera: probado con cuota 44, 60 y 74 %, la cuota REAL no se movía. El techo no era la
    // cuota: era el todo-o-nada y el sorteo a ciegas.
    //
    // Ahora se construye el offset de cada padre por los dos lados, se marca tramo a tramo lo que
    // cabe y se coge LA RACHA VIABLE MÁS LARGA. Un carril medio ocupado ya no tira la paralela
    // entera: da el trozo que queda libre, que es justo lo que hace un acompañamiento en las
    // seis —entra donde puede y dura lo que dura—. El azar sigue eligiendo, pero entre las buenas:
    // se ordena por largo y se sortea dentro del tercio de cabeza. Declarar la preferencia, no
    // convertirla en restricción: no se coge siempre la más larga, se coge una de las largas.
    if (cat === 'paralela') {
      let hecha = null;
      const cands = [];
      const cabe = [];
      for (let pi = 0; pi < trazos.length; pi++) if (trazos[pi].length >= 3) cabe.push(pi);
      // con muchos trazos no hace falta mirarlos todos: el primero siempre —es el cuerpo— y una
      // muestra del resto, para que el coste no crezca con el cuadrado del número de trazos.
      const mira = cabe.length <= 6 ? cabe
        : [cabe[0]].concat(cabe.slice(1).filter(() => rng.bool(5 / cabe.length)));
      for (const pi of mira) {
        const padre = trazos[pi];
        const choca = estorba(-1, padre);
        for (const lado of [1, -1]) {
          const full = offsetDe(padre, lado, 0, padre.length - 1);
          if (full.length < 3) continue;
          // LA RACHA MÁS LARGA de tramos que caben. `seCruza` se pregunta contra el trozo de la
          // racha en curso, no contra el offset entero: si la racha se ha cortado, lo de antes ya
          // no es este trazo y no puede cruzarse con él.
          let mejor = null, ini = -1;
          for (let i = 0; i <= full.length - 1; i++) {
            const ok = i < full.length - 1 && !choca(full[i], full[i + 1]) &&
                       !(ini >= 0 && seCruza(full.slice(ini, i + 1), full[i], full[i + 1]));
            if (ok) { if (ini < 0) ini = i; continue; }
            if (ini >= 0) {
              const trozo = full.slice(ini, i + 1), L = largoDe(trozo);
              if (!mejor || L > mejor.L) mejor = { trozo, L };
              ini = -1;
            }
          }
          if (mejor && mejor.L > W_NOM * M.dura) cands.push(mejor);
        }
      }
      if (cands.length) {
        cands.sort((a, b) => b.L - a.L);
        const top = cands.slice(0, Math.max(1, Math.ceil(cands.length / 3)));
        hecha = top[rng.int(0, top.length - 1)].trozo;
      }
      if (hecha) {
  ...
  for (let k = 0; k < trazos.length; k++) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      for (const s2 of [0, 1]) {
        ...
        // EL SUELO DE LA BANDA NO PUEDE GANARLE AL SITIO QUE HAY. `hasta` dice cuánto cabe dejando el
        // canal; el suelo de 0,42 se lo saltaba, así que en el punto más apretado las dos bandas se
        // quedaban con su 0,42 por lado y se comían el canal: 0,004 anchuras de blanco donde el cabo
        // de un apoyo muere junto a una banda que sigue. El suelo está para que la banda no se
        // deshinche —«el grosor es demasiado variable»— y para eso vale contra el campo abierto; en un
        // apretón manda el sitio, porque el margen es la regla y el grosor parejo es un gusto. Se
        // conserva un mínimo absoluto de 0,22 para que la banda no desaparezca del todo: por debajo
        // de eso el problema es de la composición y no del relleno.
